...
setwd("C:/Users/schenouf/Documents/Projet EUROHAB/Nouveau contrat/Données_REPHYTOX")
...
head(DataToxins)
dim(DataToxins)
DataToxins <- unique(DataToxins) #DataToxins is REPHYTOX data
dim(DataToxins)
...
#### Import of the fishing season table
...
DF <- DF[!is.na(DF$Groupe.de.toxines),]  #we keep only the occurrences included in the reference table
dim(DF)

#Create the variable "Toxins" according to the word document instructions. It based on regulations (official data)
...
#Create the variable "Toxicity" according to the word document instructions. It based on regulations (official data)
DF$Toxicity <- "no"
DF$Toxicity[DF$Résultat...Valeur.de.la.mesure > DF$Seuil.réglementaire] <- "yes"
table(DF$Toxicity)

#Create the variable Alert according to the word document instructions. Binary coding 
DF$Alert <- NA
...
#Definition of the secondary component "Toxicity ASPshell for ASP
...
#we delete what doesn't match with ASP 
DF$ToxicityBis[!DF$Toxins%in%"ASP"] <- NA
#and we adopt the definition of the variable Alert...
...
#Compile the phycotoxins concentrations to have only one line for each combination FishingZone-Date 
#and put the Alerts per toxin familly in column  
...
#Create the variable "AlertTot" according to the word document instructions
...
#Develop the intermediate days between sampling dates 
#the dimensional support is (taxon x zone)
...
#and develop the period on a daily basis
...
dim(FullDF)
#Integrate the table: DF
FullDF <- merge(FullDF,DF,all.x=TRUE)

#FullDF is the daily table
...
FullDF_V2 <- do.call("rbind.data.frame",lapply(unique(FullDF$FishingZone), function(x) {
  ...
  #Create the vector "Closure" day after day by browsing 'tmp'
  ...
  tmp$Closure <- CL
  #Define the "Closure" events 
  #A "Closure" is defines as a 1 preceded by a 0 in the "Closure" column for Zone variable (or a 1 at the beginning of the time series)
  tmp$ClosureEvent <- 0
  tmp$ClosureEvent[grep("01",paste(tmp$Closure[-nrow(tmp)],tmp$Closure[-1],sep=""))+1] <- 1
  #and we manage the first value if equal to 1 
  if (tmp$Closure[1]%in%1) tmp$ClosureEvent[1] <- 1
  #We add distinctions by toxin family 
  ...
  #Browse according to "Closure" and complete
  for (i in 2:nrow(tmp)) {
...
#Export of the complete table (closure calendar)
write.table(FullDF_V2,file="ClosureCalendar.txt",sep="\t",quote=FALSE,row.names=FALSE)

#Synthesis of the developed table, in terms of number and duration of closures (in days) per area and per week (alt. fishing season)
#Filtering on the fishing seasons
